...
const patternInstructionPairs = new Map([
  ...
  // TODO: Have each instruction function be a pure function that returns a
  // modified state instead of mutating the original state passed to it as
  // an argument.
  ...
  [
    /^(?:copyAndMove|cpy) \[(\d+)\]->\[(\d+)\]/i,
    (state, indexFrom, indexTo) => {
  ...
  [
    /^(?:exi?t|(?:good)?bye)/i,
    state => {
      logging.logBlue('Byeeeeee :)')
      process.exit(0)
    }
  ],
  /* Instructions requiring recursive pattern-matching */
  [
    recursiveInstruction(/^(?:while|whi) {/i),
    (state, codeBlock) => {
  ...
  [
    recursiveInstruction(/^(?:if|ift) {/i),
    (state, codeBlock) => {
      if (state.stack.top()) evaluate(codeBlock, state)
    }
  ],
  [
    recursiveInstruction(/^(?:ifNot|iff) {/i),
    (state, codeBlock) => {
      if (!state.stack.top()) {
        evaluate(codeBlock, state)
      }
  ...
  [
    recursiveInstruction(/^(?:twice|twi) {/i),
    (state, codeBlock) => {
      evaluate(codeBlock, state)
      evaluate(codeBlock, state)
    }
  ...
  [
    recursiveInstruction(/^(?:hold on|ign) <~/i, '<~', '~>'),
    (state, codeBlock) => {
