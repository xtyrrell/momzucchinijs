...
const recursivePatternInstructionPairs = new Map([
  ...
  [recursiveBlock(/^while {/i), (state, codeBlock) => {
    while (state.stack.top()) {
      evaluate(codeBlock, state)
    }
  }],
  [recursiveBlock(/^if {/i), (state, codeBlock) => {
      if (state.stack.top()) evaluate(codeBlock, state)
    }
  ],
  [recursiveBlock(/^ifNot {/i), (state, codeBlock) => {
    if (!state.stack.top()) {
      evaluate(codeBlock, state)
    }
  }],
  [recursiveBlock(/^twice {/i), (state, codeBlock) => {
      evaluate(codeBlock, state)
      evaluate(codeBlock, state)
    }
  ],
  [recursiveBlock(/^archetype <(\w+)> {/i), (state, archetypeName, codeBlock) => {
  ...
  [recursiveBlock(/^hold on <~/i, '<~', '~>'), (state, codeBlock) => {
